...
print(malechor)
print('https://github.com/kglb1/malechor.py')
def menu():
    print('1 Reverse Hash, 2 Leaked Email Lookup, 3 Instagram Bruteforce Attack')

    choice = input()
    if choice == "1":


            input_hash = input('Insert hash: ')
            api = ('detij2JCVxi86KircnB4dYlANfQ8pQ')
            hash = input_hash
            payload = {'key': {api}, 'query': {hash}}
            request = requests.get('https://hashes.org/api.php?key', params=payload)
            data = request.json()
            print(data)
    ...
    if choice == "h":
        print("add passwords to brute in pass.txt")
